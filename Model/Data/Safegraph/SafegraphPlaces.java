...
public class SafegraphPlaces implements Serializable {
    ...
    public String name;
    public Categories categories;//MAIN
    public Brands brands;//MAIN
...
//        System.out.println("$$$");
    ...
    public void preprocessMonthCorePlacesNew(String source, String destination, String patternName, boolean isParallel, int numCPU) {
        String[] split=patternName.split("_");
        name = "core_poi_"+split[1]+"_"+split[2];
        placesRecords = new ArrayList();
        categories = new Categories();
        brands = new Brands();
        ...
        for (int i = 0; i < cSVfileList.length; i++) {
            ...
            if (isRawBinFound == false) {
                ...
                try {
                    recordsLocal = readData(cSVfileList[i].getCanonicalPath(), isParallel, numCPU);
                    placesRecords = recordsLocal;
                    Safegraph.saveSafegraphPlacesKryo(source + "/ProcessedData_" + cSVfileList[i].getName(), this);
                    placesRecords.clear();
                } catch (IOException ex) {
                    Logger.getLogger(SafegraphPlaces.class.getName()).log(Level.SEVERE, null, ex);
                }
        ...
        cSVfileList = directory.listFiles(cSVFilesFilter);
        File[] binFileListRecheck = directory.listFiles(binFilesFilter);
        placesRecords = new ArrayList();
        for (int i = 0; i < binFileListRecheck.length; i++) {
            for (int j = 0; j < cSVfileList.length; j++) {
                if (binFileListRecheck[i].getName().contains(cSVfileList[j].getName())) {
                    SafegraphPlaces safegraphPlaces = Safegraph.loadSafegraphPlacesKryo(binFileListRecheck[i].getPath());
                    placesRecords.addAll(safegraphPlaces.placesRecords);
                    safegraphPlaces = null;
                    System.out.println("Data read: " + i);
...
//        System.out.println("***");
        Collections.sort(placesRecords);
        Safegraph.saveSafegraphPlacesKryo(destination + "/processedData", this);
        for (int i = 0; i < binFileListRecheck.length; i++) {
            for (int j = 0; j < cSVfileList.length; j++) {
                if (binFileListRecheck[i].getName().contains(cSVfileList[j].getName())) {
                    binFileListRecheck[i].delete();
...
//        System.out.println("$$$");
    ...
    public ArrayList<SafegraphPlace> readDataNew(String fileName, boolean isParallel, int numCPU) {
