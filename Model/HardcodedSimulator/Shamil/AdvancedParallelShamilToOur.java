...
public class AdvancedParallelShamilToOur extends ParallelProcessor {
    ...
    public AdvancedParallelShamilToOur(MainModel parent, ArrayList<Person> people, int startIndex, int endIndex) {
        super(parent, people, startIndex, endIndex);
        myRunnable = new Runnable() {
            @Override
            public void run() {
                for (int i = myStartIndex; i < myEndIndex; i++) {
                    ShamilSimulatorController.convertShamilToOurPerson(people.get(i));
                }
            }
        };
    }
